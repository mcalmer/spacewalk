...
function git_package_defs() {
  ...
  test -n "$PACKAGE" || {
    PACKAGE=$(ls "$GIT_DIR"/rel-eng/packages/ \
              | grep -v -x -e apt-spacewalk -e cobbler-loaders -e jabberd-selinux -e oracle-rhnsat-selinux \
                     -e oracle-selinux -e oracle-server-admin -e oracle-server-scripts -e oracle-xe-selinux \
                     -e rhnclient -e spacewalk-selinux -e spacewalk-proxy-selinux -e yum-rhn-plugin \
                     -e simple-xml -e perl-Net-INET6Glue -e bootstrap-datepicker -e redstone-xmlrpc -e patternfly1 \
                     -e c-ares19 -e nodejs-packaging -e cx_Oracle -e perl-DBD-Oracle -e select2-bootstrap-css -e jquery-ui \
                     -e roboto -e simple-core -e cobbler20 -e heirloom-pkgtools -e susestudio-java-client -e ace-editor \
                     -e quartz -e nodejs -e scons -e font-awesome -e bootstrap -e momentjs -e select2 -e nutch -e libuv \
                     -e spacewalk-repo -e ycssmin -e v8 -e dwr -e gyp -e stringtree-json -e jpam -e tanukiwrapper \
                     -e perl-Mail-RFC822-Address -e jquery-timepicker -e tito -e http-parser -e spacewalk-jpp-workaround \
                     -e mod_perl -e pwstrength-bootstrap -e oracle-instantclient-selinux -e nodejs-less -e spacewalk-repo )
  }
  for N in $PACKAGE; do
